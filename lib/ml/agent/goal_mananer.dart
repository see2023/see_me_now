...
class GoalManager extends GetxController {
  ...
  runOnce() async {
    ...
    if (agentData.runningActionId > 0) {
      // check action timeout
      Log.log.fine('action ${agentData.runningActionId} pending');
      // execute next action
      await executeNextAction(agentData.runningTaskId);
      return;
    }
